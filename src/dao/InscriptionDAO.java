...
public class InscriptionDAO {
    ...
    public static List<ChambreA2> getA2FemaleReservedRooms() throws Exception {
        ...
        conn.setAutoCommit(true);
        PreparedStatement ps = conn.prepareStatement("SELECt  Chambre.Idchambre,CodeChambre,IdEtage,IdChambreA2,EtatPlace1,EtatPlace2 from chambre,chambreA2,Inscription WHERE Chambre.IdChambre = chambreA2.IdChambre and Inscription.IdChambre = chambre.IdChambre and Inscription.current=true and (Select count(IdChambre) FROM Inscription WHERE Inscription.current=true )<2 and IdEtage IN(SELECT IdEtage FROM Etage WHERE IdBatiment IN(SELECT IdBatiment FROM Batiment WHERE gender='F'))");
        ResultSet rs = ps.executeQuery();
        while(rs.next())
        {
